...
4. (Optional) DisableYouCompleteMe by commenting out "Plugin 'Valloric/YouCompleteMe'" in ~/.vimrc and goto next step

  4.1 Install YouCompleteMe
    ...
    # My setting
    $ LD_LIBRARY_PATH=$(equery f sys-devel/clang | sed -rn '/libclang.so$/s#/libclang.so##p') ./install.py --clang-completer --system-libclang --gocode-completer

    # check library
    $ ldd ~/.vim/bundle/YouCompleteMe/third_party/ycmd/ycm_core.so

4. (Optional) Disable vim-go by comment out "Plugin 'fatih/vim-go'" in ~/.vimrc
   If you would like to enable vim-go, please launch vim and run :GoInstallBinaries
